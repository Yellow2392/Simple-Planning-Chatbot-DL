...
def get_system_prompt(domain: str) -> str:
    if domain == "craves":
        return (
            ...
            "The user prompt ends with '[THOUGHT]'. In max 2 sentences, map the goals to the necessary actions.\n"
            "Then write exactly '[THOUGHT END]' on its own line, then '[PLAN]'.\n\n"
            "CRITICAL HEURISTICS FOR THIS DOMAIN:\n"
            "1. THE FIRST MOVE (CRITICAL): Look strictly at [STEP0 HINTS]. If 'attackable0' is NOT empty, your first action MUST be '(attack X)'. If 'attackable0' IS empty, your first action MUST be '(feast X Z)' using the pair from 'feastable0'.\n"
            "2. THE 'OVERCOME' TARGET: To achieve a goal of 'X craves Y', the final step for that target is ALWAYS '(overcome X Y)'.\n"
            "3. THE SUCCUMB TRAP: NEVER perform '(succumb X)' immediately before '(overcome X Y)'. Succumb destroys the 'Pain' you need for Overcome! \n"
            "4. WHEN TO SUCCUMB: Only use '(succumb X)' AFTER an overcome, or if you desperately need to restore 'Harmony' to perform a completely DIFFERENT '(attack)' or '(feast)'.\n"
